...
import { DELETE_ELECTION_PROGRESS, DELETE_PARTY_PROGRESS, GET_ELECTION_PROGRESS, UPDATE_PARTY_PROGRESS } from '../../../Redux/Admin/action/action';
...
const Elections = () => {
  ...
  const allElections = useSelector((state) => state.userReducer.elections)
  ...
  const handleDelete = (id, index) => {
    dispatch({ type: DELETE_ELECTION_PROGRESS, payload: { id, index } })
  }

  const handleView = (index) => {
    console.log(index, allElections[index]);
    setView(allElections[index]);
    setHide("none")
    setShow("block");
  }
        ...
        {/* table for user data */}
        {
          allElections ? (
            <div className="relative overflow-x-auto shadow-md sm:rounded-lg w-full m-auto mt-14">
              {
                <table className="w-[70%] m-auto text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 my-14 p-2">
                  ...
                  <tbody>
                    {
                      allElections.filter((item) => {
                        return (
                          searchInput.toLocaleLowerCase() === "" ? item : item.ElectionName.toLocaleLowerCase().includes(searchInput)
                        );
                      })
                        .map((item, ind) => (
                          <tr key={ind} className="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700 lg:text-[18px] sm:text-[16px] text-[14px]">
                            <td className="px-4 py-3">{item.ElectionName}</td>
